...
#define US(t) (uint32_t)((uint64_t)(t)*1000000/rte_get_tsc_hz())
...
#if 0
    if (d->crc != rte_raw_cksum(d->data, d->len)){
        printf("data msg crc=%u err!\n", c->crc);
    }
#endif
    g_rx_ttis++;
    ...
    //printf("TSC: %8u %8u %8u %8u %8u %8u %8u %8u\n", c->tsc[0], c->tsc[1], c->tsc[2], c->tsc[3], c->tsc[4], c->tsc[5], c->tsc[6], c->tsc[7]);
...
#endif
    pc802_create_tx_queue( port, PC802_TRAFFIC_OAM, 32*1024, 128, 64);
    pc802_create_rx_queue( port, PC802_TRAFFIC_OAM, 32*1024, 128, 64);
...
int test_cell_perf(uint16_t count)
{
    ...
    uint32_t group=0, no=0, gourp_size = 2048;
    uint64_t sn=0;
    uint64_t tx_tsc=0, tsc=0, dt2_0=0, max_dt2_0=0, dt7_5=0, max_dt7_5=0, bytes=0, tx_ttis=0, rx_ttis=0;
    uint64_t tti_tsc, hz = rte_get_tsc_hz();
    void *ctrl_buf[8] = {NULL}, *data_buf[8] = {NULL};
    uint32_t *buf;

    tti_tsc = 500*hz/1000000;
    printf("start latency test: count=%u gourp_size=%u hz=%lu\n\n", count, gourp_size, hz);
    ...
    while ( group < count ) {
        ...
        if ( (sn - rx_ttis > 0) || (rte_rdtsc()-tx_tsc < tti_tsc) )
            continue;
        tx_tsc = rte_rdtsc();
        ...
        buf = (uint32_t *)data_buf[rte_rand()%8];
        ...
        buf = (uint32_t *)ctrl_buf[rte_rand()%8];
        buf[rte_rand()%(ctrl_len/4)] = rte_rdtsc();
        ctrl_msg->crc = rte_raw_cksum(buf, ctrl_len);
        ctrl_msg->tsc[ctrl_msg->tscs++] = rte_rdtsc();

        memcpy(ctrl_msg->data, buf, ctrl_len);
        ctrl_msg->tsc[ctrl_msg->tscs++] = rte_rdtsc();
