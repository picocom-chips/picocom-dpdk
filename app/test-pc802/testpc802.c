...
static int case10(void)
{
    int re;
    uint32_t N;
    uint32_t *a = alloc_tx_blk(QID_CTRL[3]);
    if (NULL == a) return -1;

    produce_dl_src_data(a, QID_CTRL[3]);
    N = sizeof(uint32_t) * (a[1] + 2);
    set_blk_attr(a, N, 2, 1);
    tx_blks(QID_CTRL[3], &a, 1);

    uint32_t *b;
    while (0 == rx_blks(QID_CTRL[3], &b, 1));
    uint32_t length = 0;
    uint8_t type, eop = 0;
    get_blk_attr(b, &length, &type, &eop);
    if ((type != 2) || (eop != 1))
...
static int case110(void)
{
    ...
    PCXX_CALL(pcxxSendStart, g_pc802_index, 3 );
    RTE_ASSERT(0 == pcxxCtrlAlloc(&a, &avail, g_pc802_index, 3));
    A = (uint32_t *)a;
    produce_dl_src_data(A, QID_CTRL[3]);
    N = sizeof(uint32_t) * (A[1] + 2);
    pcxxCtrlSend(a, N, g_pc802_index, 3);
    PCXX_CALL(pcxxSendEnd,g_pc802_index, 3);

    while (-1 == PCXX_CALL(pcxxCtrlRecv,g_pc802_index, 3));
    int re = atl_test_result[g_pc802_index][3];
    atl_test_result[g_pc802_index][3] = 0;
    return re;
}
...
static int case11(void)
{
    int re;
    uint32_t N;
    uint32_t *a = alloc_tx_blk(QID_CTRL[4]);
    if (NULL == a) return -1;

    produce_dl_src_data(a, QID_CTRL[4]);
    N = sizeof(uint32_t) * (a[1] + 2);
    set_blk_attr(a, N, 2, 1);
    tx_blks(QID_CTRL[4], &a, 1);

    uint32_t *b;
    while (0 == rx_blks(QID_CTRL[4], &b, 1));
    uint32_t length = 0;
    uint8_t type, eop = 0;
    get_blk_attr(b, &length, &type, &eop);
    if ((type != 2) || (eop != 1))
...
static int case111(void)
{
    ...
    PCXX_CALL(pcxxSendStart, g_pc802_index, 4);
    RTE_ASSERT(0 == pcxxCtrlAlloc(&a, &avail, g_pc802_index, 4));
    A = (uint32_t *)a;
    produce_dl_src_data(A, QID_CTRL[4]);
    N = sizeof(uint32_t) * (A[1] + 2);
    pcxxCtrlSend(a, N, g_pc802_index, 4);
    PCXX_CALL(pcxxSendEnd,g_pc802_index, 4);

    while (-1 == PCXX_CALL(pcxxCtrlRecv,g_pc802_index, 4));
    int re = atl_test_result[g_pc802_index][4];
    atl_test_result[g_pc802_index][4] = 0;
    return re;
}
...
static int case_n802(void)
{
    ...
    while (1) {
        ...
        diag = case10();
        return_if_fail(10, diag, k);
        diag = case110();
        return_if_fail(110, diag, k);
        diag = case11();
        return_if_fail(11, diag, k);
        diag = case111();
        return_if_fail(111, diag, k);
        m++;
        k++;
        if (TEST_PC802_DISP_LOOP_NUM == m) {
