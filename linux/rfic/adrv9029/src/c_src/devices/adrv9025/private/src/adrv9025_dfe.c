...
#define ADRV9025_CLGC_CONFIG_OFFSET_BYTES 56u
    static const uint8_t                   PA_PROTECTION_EN                              = 0u;
                                      ...
                                      uint8_t                                   useEnhanced)
{
    ...
    static const uint8_t ARM_ERR_CODE = 0x0E;
    static const uint8_t DPD_TRACKING_CONFIG_LEN = 108u;
    ...
    uint8_t armData[110] = { 0 }; /* 110 should be DPD_TRACKING_CONFIG_LEN + 2 */
    uint8_t cmdStatusByte = 0;
    uint8_t i = 0;
    ...
    else
    {
        ...
        armData[104] = enhancedTrackingConfig->enableWidebandRegularization;

        fpSerializerPtr = (uint32_t*)(&(enhancedTrackingConfig->widebandRegularizationFactor));
        fpSerializerTmp = *fpSerializerPtr;
        armData[106] = (uint8_t)(fpSerializerTmp & 0xFF);
        armData[107] = (uint8_t)((fpSerializerTmp >> 8) & 0xFF);
        armData[108] = (uint8_t)((fpSerializerTmp >> 16) & 0xFF);
        armData[109] = (uint8_t)((fpSerializerTmp >> 24) & 0xFF);
    }
    for (i = 0; i < ADRV9025_NUMBER_OF_TX_CHANNELS; i++)
    {
        if ((txChannelMask & (0x1 << i)) != 0)
                                      ...
                                      uint8_t                                   useEnhanced)
{
    ...
    uint8_t armData[108] = { 0 }; /* 108 should be DPD_TRACKING_CONFIG_LEN */
    uint8_t cmdStatusByte  = 0;
    uint8_t i              = 0;
    float*  fpDeserializerPtr = NULL;
    ...
    else
    {
       ADI_NULL_PTR_RETURN(&device->common, enhancedTrackingConfig);
        ...
        enhancedTrackingConfig->enableWidebandRegularization = armData[102];

        fpDeserializerTmp = (uint32_t)((uint32_t)armData[104] |
                                       (uint32_t)armData[105] << 8 |
                                       (uint32_t)armData[106] << 16 |
                                       (uint32_t)armData[107] << 24);
