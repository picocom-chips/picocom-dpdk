...
* 
* \retval ADRV9010_ Recovery action for log reset
* \retval ADRV9010_ACT_ERR_CHECK_PARAM Recovery action for bad parameter check
* \retval ADRV9010_ACT_ERR_RESET_SPI Recovery action for SPI reset required
* \retval ADRV9010_ACT_NO_ACTION Function completed successfully, no action required
*
*/
...
//
//    if (device->common.cacheInfo.rdCacheState == ADRV9010_RD_BF_CACHE_ON)
//    {
//        recoveryAction = fpga9010_BfReadCacheFlush(device, &rdData, &numRdBytes);
...
* \param device Pointer to the ADRV9010 device data structure.
*
* \retval ADRV9010_ACT_WARN_RESET_LOG Recovery action for log reset
* \retval ADRV9010_ACT_ERR_CHECK_PARAM Recovery action for bad parameter check
* \retval ADRV9010_ACT_ERR_RESET_SPI Recovery action for SPI reset required
* \retval ADRV9010_ACT_NO_ACTION Function completed successfully, no action required
*
...
//    if (device->common.cacheInfo.wrCacheState == ADRV9010_WR_BF_CACHE_ON)
//    {
//        /*Ensures the cache is empty if bitField Level Caching is on*/
//        
//        recoveryAction = fpga9010_BfWriteCacheFlush(device);
//        ADI_ERROR_RETURN(device->common.error.newAction);
...
* \dep_begin
* \dep{device->common.cacheInfo}
* \dep{ADRV9010_BF_WRCACHE_ARRAY_TRIP_SIZE}
* \dep{SPIARRAYSIZE}
* \dep_end
*
* \param device Pointer to the ADRV9010 device data structure
...
*
*/
int32_t fpga9010_BfRegisterWrite(adi_fpga9010_Device_t *device,
                             uint32_t addr,
                             uint32_t regVal)
{
    int32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
...
//
//    if (device->common.cacheInfo.wrCacheState == ADRV9010_WR_GBL_MERGE_ON)
//    {
...
//    }
//
//    if ((device->common.cacheInfo.wrCacheState != ADRV9010_WR_CACHE_OFF) && 
...
//        if ((tmp_wrCacheIdx++) >= ADRV9010_BF_WRCACHE_ARRAY_TRIP_SIZE)
//        {
...
//        {
//            recoveryAction = adi_fpga9010_hal_SpiByteWrite(device, addr, regVal);
//            ADI_ERROR_RETURN(device->common.error.newAction);
//        }
//    }
    adi_fpga9010_RegisterWrite(device, addr, regVal); 
    ADI_ERROR_RETURN(device->common.error.newAction);  
    return recoveryAction;
}
...
* \dep_begin
* \dep{device->common.cacheInfo}
* \dep{ADRV9010_BF_RDCACHE_ARRAY_TRIP_SIZE}
* \dep{SPIARRAYSIZE}
* \dep_end
*
* \param device Pointer to the ADRV9010 device data structure
...
*
*/
int32_t fpga9010_BfRegisterRead(adi_fpga9010_Device_t *device,
                            uint32_t addr,
                            uint32_t *regVal)
{
    int32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
...
//    else
//    {
//       // recoveryAction = adi_fpga9010_hal_SpiByteRead(device, addr, regVal);

    //int32_t adi_fpga9010_RegisterRead(adi_fpga9010_Device_t *device, uint32_t offset, uint32_t *data)
    //ToDo replace theis with fieldRead.

    adi_fpga9010_RegisterRead(device, addr, regVal);   
    ADI_ERROR_RETURN(device->common.error.newAction);
//    }
    return recoveryAction;
}
...
*
*/
int32_t fpga9010_BfFieldWrite(adi_fpga9010_Device_t *device,
                              uint32_t addr,
                              uint32_t fieldVal,
                              uint32_t mask,
                              uint8_t startBit)
...
//    if (device->common.cacheInfo.wrCacheState > ADRV9010_WR_HW_RMW_ON)
//    {
...
//            }
//        }
//        if ((device->common.cacheInfo.wrCacheState > ADRV9010_WR_CACHE_OFF) & !addrFoundInCache)
//        {
//            /* if caching is on or
...
//        {
//            if (device->common.cacheInfo.wrCacheIdx >= ADRV9010_BF_WRCACHE_ARRAY_TRIP_SIZE)
//            {
...
//                ADI_ERROR_RETURN(device->common.error.newAction);
//            }
//            recoveryAction = adi_fpga9010_hal_SpiByteWrite(device, addr, newRegVal);
//            ADI_ERROR_RETURN(device->common.error.newAction);
//        }
//        else
//        {
...
*
*/
int32_t fpga9010_BfFieldRead(adi_fpga9010_Device_t *device,
                             uint32_t addr,
                             uint32_t *fieldVal,
                             uint32_t mask,
                             uint8_t startBit)
...
//        if ((rdCacheIdx++) >= ADRV9010_BF_RDCACHE_ARRAY_TRIP_SIZE)
//        {
...
//        /* Implements immediate spi transaction call only used when caching is turned off. */
        //recoveryAction = adi_fpga9010_hal_SpiFieldRead(device, addr, fieldVal, mask, startBit);

        startBit = startBit & 0x3F;  /* remove the End Of BitField Marker */

        recoveryAction = adi_fpga9010_RegisterFieldRead(device, addr, fieldVal, mask, startBit);
...
*
* \retval ADI_FPGA90109010_ACT_WARN_RESET_LOG Recovery action for log reset
* \retval ADI_FPGA90109010_ACT_ERR_CHECK_PARAM Recovery action for bad parameter check
* \retval ADI_FPGA90109010_ACT_ERR_RESET_SPI Recovery action for SPI reset required
* \retval ADI_FPGA90109010_ACT_NO_ACTION Function completed successfully, no action required
*
*/
int32_t fpga9010_BfReadAssembleData(adi_fpga9010_Device_t *device,
                                    uint32_t *rxBfData,
                                    uint8_t numRxBfData,
                                    uint64_t *rxData)
{
...
//    if (device->common.cacheInfo.rdCacheState == ADRV9010_RD_BF_CACHE_ON)
//    {
//        recoveryAction = fpga9010_BfReadCacheFlush(device, rxData, &numRxBitFields);
//        ADI_ERROR_RETURN(device->common.error.newAction);
//    }
//    else if (device->common.cacheInfo.rdCacheState == ADRV9010_RD_CACHE_OFF)
//    {
...
*
* \dep_begin
* \dep{ADRV9010_BF_RDCACHE_ARRAY_SIZE}
* \dep{SPIARRAYSIZE}
* \dep_end
*
* \param device Pointer to the ADRV9010 device data structure
...
*
*/
int32_t fpga9010_BfReadCacheFlush(adi_fpga9010_Device_t *device,
                                  uint64_t *rdData,
...
//    uint32_t i = 0;
//    uint16_t addr = 0;
//    uint16_t rdAddr[ADRV9010_BF_RDCACHE_ARRAY_SIZE] = { 0 };
//
//    uint8_t regVal = 0;
//    uint8_t mask = 0;
//    uint8_t startBit = 0;
...
//
//    uint8_t regData[ADRV9010_BF_RDCACHE_ARRAY_SIZE] = { 0 };
//    int32_t halError = ADI_COMMON_HAL_OK;
...
//    *numRdBitFields = 0;
//
//    if (device->common.cacheInfo.rdCacheIdx > ADRV9010_BF_RDCACHE_ARRAY_SIZE)
//    {
//        ADI_ERROR_REPORT(&device->common,  2, ADI_COMMON_ERRSRC_DEVICEBF, 
//                     ADI_ADRV9010_ACT_ERR_FLUSH_CACHE,
//                     NULL, 
...
//           into a address array suitable for ADRV9010HAL_spiReadBytes*/
//        rdAddr[i] = device->common.cacheInfo.rdCache[i] >> SPI_ADDR_SIZE;
//    }
//
//    /* send the array and receive an array of register values */
//    adi_fpga9010_hal_SpiBytesRead(device, &rdAddr[0], &regData[0], device->common.cacheInfo.rdCacheIdx);
//    ADI_ERROR_RETURN(device->common.error.newAction);
//
...
* \dep_begin
* \dep{device->common.cacheInfo}
* \dep{ADRV9010_BF_WRCACHE_ARRAY_SIZE}
* \dep_end
*
* \param device Pointer to the ADRV9010 device data structure
*
...
//TODO: remove this if not used
//    uint32_t i = 0;
//    uint16_t wrAddr[ADRV9010_BF_WRCACHE_ARRAY_SIZE] = { 0 };
//    uint8_t wrData[ADRV9010_BF_WRCACHE_ARRAY_SIZE] = { 0 }; 
//    int32_t halError = ADI_COMMON_HAL_OK;
...
//        recoveryAction = adi_fpga9010_hal_SpiBytesWrite(device, &wrAddr[0], 
//                                                   &wrData[0], 
...
*
*/
//int32_t fpga9010_BfWriteCacheEn(adi_fpga9010_Device_t *device,
//                                fpga9010_Bf_WrCache_State_e wrCacheState)
//{
//    int32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
//    int32_t halError = ADI_COMMON_HAL_OK;
...
*
*/
//int32_t fpga9010_BfReadCacheEn(adi_fpga9010_Device_t *device,
//                               fpga9010_Bf_RdCache_State_e rdCacheState)
//{
//    int32_t recoveryAction = ADI_COMMON_ACT_NO_ACTION;
//    int32_t halError = ADI_COMMON_HAL_OK;
