...
int32_t ads9_BbicOpen(void* devHalCfg)
{
    ...
    {
        halCfg->bbicCfg.coreRegsCfg.fd = open(halCfg->bbicCfg.coreRegsCfg.drvName,
                                              O_RDWR);
        ...
        halCfg->bbicCfg.coreRegsCfg.mappedMemoryPtr = (uintptr_t)mmap(NULL,
                                                                      halCfg->bbicCfg.coreRegsCfg.mapSize,
                                                                      PROT_READ | PROT_WRITE,
                                                                      MAP_SHARED,
                                                                      halCfg->bbicCfg.coreRegsCfg.fd,
                                                                      0);
    ...
    {
        halCfg->bbicCfg.ramCfg.fd = open(halCfg->bbicCfg.ramCfg.drvName,
                                         O_RDWR);
        ...
        /* offset must be page aligned */
        halCfg->bbicCfg.ramCfg.mappedMemoryPtr = (uintptr_t)mmap(NULL,
                                                                 halCfg->bbicCfg.ramCfg.mapSize,
                                                                 PROT_READ | PROT_WRITE,
                                                                 MAP_SHARED,
                                                                 halCfg->bbicCfg.ramCfg.fd,
                                                                 0);
    ...
    {
        halCfg->bbicCfg.spiAdvRegsCfg.fd = open(halCfg->bbicCfg.spiAdvRegsCfg.drvName,
                                                O_RDWR);
        ...
        /* offset must be page aligned */
        halCfg->bbicCfg.spiAdvRegsCfg.mappedMemoryPtr = (uintptr_t)mmap(NULL,
                                                                        halCfg->bbicCfg.spiAdvRegsCfg.mapSize,
                                                                        PROT_READ | PROT_WRITE,
                                                                        MAP_SHARED,
                                                                        halCfg->bbicCfg.spiAdvRegsCfg.fd,
                                                                        0);
...
int32_t ads9_BbicClose(void* devHalCfg)
{
    ...
    if (((halCfg->bbicCfg.interfaceEnabled & ADI_HAL_BBIC_CORE) > 0) && halCfg->bbicCfg.coreRegsCfg.fd != 0)
    {
        halError = munmap((void *)halCfg->bbicCfg.coreRegsCfg.mappedMemoryPtr,
                          halCfg->bbicCfg.coreRegsCfg.mapSize);
        if (halError < 0)
        {
            return (int32_t)ADI_HAL_BBICCTRL_FAIL;
    ...
    if (((halCfg->bbicCfg.interfaceEnabled & ADI_HAL_BBIC_RAM) > 0) && halCfg->bbicCfg.ramCfg.fd != 0)
    {
        halError = munmap((void *)halCfg->bbicCfg.ramCfg.mappedMemoryPtr,
                          halCfg->bbicCfg.ramCfg.mapSize);
        if (halError < 0)
        {
            return (int32_t)ADI_HAL_BBICCTRL_FAIL;
    ...
    if (((halCfg->bbicCfg.interfaceEnabled & ADI_HAL_BBIC_SPI) > 0) && halCfg->bbicCfg.spiAdvRegsCfg.fd != 0)
    {
        halError = munmap((void *)halCfg->bbicCfg.spiAdvRegsCfg.mappedMemoryPtr,
                          halCfg->bbicCfg.spiAdvRegsCfg.mapSize);
        if (halError < 0)
        {
            return (int32_t)ADI_HAL_BBICCTRL_FAIL;
...
{
    (void)(devHalCfg);
    /* ADI ADS9 platform does not require any BBIC register/control init */
                              ...
                              uint32_t* data)
{
    ...
    halError = ads9_fpga_AxiReadWrite(devHalCfg,
                                      READ_REG,
                                      addr,
                                      data);
                               ...
                               uint32_t numDataWords)
{
    ...
    halError = ads9_fpga_AxiBulkReadWrite(devHalCfg,
                                          READ_REG,
                                          addr,
                                          data,
                                          numDataWords);
                               ...
                               uint32_t* data)
{
    ...
    if ((addr >= ADS9_FPGA_UIO2_START_ADDRESS) &&
        (addr <= ADS9_FPGA_UIO2_STOP_ADDRESS))
    {
        offsetAddr = addr - ADS9_FPGA_UIO2_START_ADDRESS;
        axiCfg     = &halCfg->bbicCfg.spiAdvRegsCfg;
    }
    else if ((addr >= ADS9_FPGA_UIO0_START_ADDRESS) &&
        (addr <= ADS9_FPGA_UIO0_STOP_ADDRESS))
    {
        offsetAddr = addr - ADS9_FPGA_UIO0_START_ADDRESS;
...
* \param data the data to write to address addr.
* \param numData number of data words (unit32_t).
*
* \retval ADI_HAL_OK function completed successfully, no action required
* \retval ADI_HAL_NULL_PTR the function has been called with a null pointer
                                   ...
                                   const uint32_t numData)
{
    ...
    if (writeFlag)
    {
        memcpy((void *)((uintptr_t)ptr + addr),
               data,
               numData * sizeof(uint32_t));
    }
    else
    {
        memcpy(data,
               (void *)((uintptr_t)ptr + addr),
               numData * sizeof(uint32_t));
    }
