...
/*!
* \file adi_fpga9010_axi_adv_spi.c
* \brief Contains FPGA9010 AXI Advanced SPI function implementations
*
...
int32_t adi_fpga9010_AxiAdvSpiCfgSet(adi_fpga9010_Device_t *device, adi_fpga9010_AxiAdvSpiFeatConfig_t *advSpiCfg, uint8_t slaveSelect)
{
    ...
    ADI_NULL_PTR_RETURN(&device->common, advSpiCfg);
...
int32_t adi_fpga9010_AxiAdvSpiChipSelectAll(adi_fpga9010_Device_t *device, uint8_t enable)
{
    ...
    if (enable == ADI_TRUE)
    {
        ...
        recoveryAction = adi_hal_BbicRegisterWrite(device->common.devHalInfo, spissrAddress, ~TOK_CS);
        ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, recoveryAction, ADI_COMMON_ACT_ERR_CHECK_PARAM, spissrAddress, "Error selecting all Tokelau devices");
        ADI_ERROR_RETURN(device->common.error.newAction);
        ...
        ADI_ERROR_REPORT(&device->common, ADI_COMMON_ERRSRC_API, recoveryAction, ADI_COMMON_ACT_ERR_CHECK_PARAM, spicrAddress, "Error writing spicrAddress");
        ADI_ERROR_RETURN(device->common.error.newAction);
    }
    else
    {
