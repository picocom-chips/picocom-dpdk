...
int32_t ads9_SpiOpen(void* devHalCfg)
{
    ...
    if (spiCfg->fd <= 0)
    {
        spiCfg->fd = open(spiCfg->spidevName,
                          O_RDWR);
                         ...
                         uint32_t      numTxBytes)
{
    ...
    if (halCfg->spiCfg.spiActionDisable == 0)
    {
        ...
        {
            /* Reset Tx/Rx FIFO */
            halError = ads9_BbicRegisterWrite(devHalCfg,
            ...
            for (k = 0; k < (numOfChunks + 1); k++)
            {
                ...
                for (i = 0; i < WAIT_FIFO_OCCUPANCY; i++)
                {
                    ...
                    halError = ads9_BbicRegisterRead(devHalCfg,
                                                     SPI_TXFIFO_OCY_REGISTER,
                                                     &readData);
                    if (halError != (int32_t)ADI_HAL_OK)
                    {
                        halError = (int32_t)ADI_HAL_SPI_FAIL;
